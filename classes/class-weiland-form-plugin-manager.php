...
class Weiland_Form_Plugin_Manager
{
    ...
    public function activateFrontend($attributes)
    {

        $controllerName = FALSE;
        $actionName = FALSE;
        foreach ($attributes as $controllerShortName => $actionShortName) {
            $controllerClassName = 'Weiland_Form_Frontend_' . ucfirst($controllerShortName) . '_Controller';
            if (class_exists($controllerClassName)) {
                $controllerName = $controllerClassName;
                if (method_exists($controllerName, $actionShortName . 'Action')) {
        ...
        $dispatcher = new Weiland_Form_Frontend_Dispatcher();
        $dispatcher->dispatch($controllerName, $actionName);

        /**
         * @todo integrate in controller where needed
         *
    ...
    public function autoload($className)
    {
        ...
        if (stristr($className, 'view') !== false) {
            $className = mb_strtolower($className);
            $className = str_replace('_', '-', $className);
            if (file_exists(WEILANDT_PATH . "/classes/view/class-" . $className . ".php")) {
                require_once(WEILANDT_PATH . "/classes/view/class-" . $className . ".php");
            }
        }
    ...
    function  weilandt_form_db()
    {
        ...
        $pl_weilandt_form_assignment = $wpdb->prefix . "pl_weilandt_form_assignment";
        $charset_collate = $wpdb->get_charset_collate();

        $sql5 = 'CREATE TABLE IF NOT EXISTS ' . $pl_weilandt_form_assignment . '  (
  		  id INT NOT NULL AUTO_INCREMENT,
