...
class Twig_Extension_Core extends Twig_Extension {
	...
	public function getFilters() {
		$filters = array(
			...
			new Twig_SimpleFilter( 'capitalize',
	...
	public function getFunctions() {
		return array(
			...
			new Twig_SimpleFunction( 'include',
			                         'twig_include',
			                         array(
				                         'needs_environment' => TRUE,
				                         'needs_context'     => TRUE,
			...
			new Twig_SimpleFunction( 'source',
			                         'twig_source',
	...
	public function getTests() {
		return array(
			...
			new Twig_SimpleTest( 'divisibleby',
			                     NULL,
			                     array(
			...
			new Twig_SimpleTest( 'divisible by',
			                     NULL,
...
function twig_replace_filter( $str, $from, $to = NULL ) {
	if ( $from instanceof Traversable ) {
		...
		@trigger_error( 'Using "replace" with character by character replacement is deprecated since version 1.22 and will be removed in Twig 2.0',
		                E_USER_DEPRECATED );
	...
	switch ( $strategy ) {
		...
		default:
			...
			$validStrategies = implode( ', ',
			                            array_merge( array( 'html', 'js', 'url', 'css', 'html_attr' ),
			                                         array_keys( $escapers ) ) );

			throw new Twig_Error_Runtime( sprintf( 'Invalid escaping strategy "%s" (valid ones: %s).',
			                                       $strategy,
			                                       $validStrategies ) );
	}
}
...
function twig_array_batch( $items, $size, $fill = NULL ) {
	...
	if ( NULL !== $fill && ! empty( $result ) ) {
		...
		if ( $fillCount = $size - count( $result[ $last ] ) ) {
			$result[ $last ] = array_merge( $result[ $last ],
