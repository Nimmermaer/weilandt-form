	...
	protected function getArguments( $callable, $arguments ) {
		...
		if ( ! $callable ) {
			if ( $named ) {
				...
				$message = sprintf( 'Arbitrary positional arguments are not supported for %s "%s".',
				                    $callType,
				                    $callName );
			}
		...
		foreach ( $callableParameters as $callableParameter ) {
			...
			if ( array_key_exists( $name, $parameters ) ) {
				if ( array_key_exists( $pos, $parameters ) ) {
					throw new Twig_Error_Syntax( sprintf( 'Argument "%s" is defined twice for %s "%s".',
					                                      $name,
					                                      $callType,
					                                      $callName ) );
				}
				...
				if ( ! empty( $missingArguments ) ) {
					throw new Twig_Error_Syntax( sprintf( 'Argument "%s" could not be assigned for %s "%s(%s)" because it is mapped to an internal PHP function which cannot determine default value for optional argument%s "%s".',
					                                      $name,
					                                      $callType,
					                                      $callName,
					                                      implode( ', ', $names ),
				...
				throw new Twig_Error_Syntax( sprintf( 'Value for argument "%s" is required for %s "%s".',
				                                      $name,
				                                      $callType,
				                                      $callName ) );
			}
		...
		if ( ! empty( $parameters ) ) {
			...
			throw new Twig_Error_Syntax( sprintf( 'Unknown argument%s "%s" for %s "%s(%s)".',
	...
	private function getCallableParameters( $callable, $isVariadic ) {
		...
		if ( $isVariadic ) {
			...
			if ( $argument && $argument->isArray() && $argument->isDefaultValueAvailable() &&
			     array() === $argument->getDefaultValue()
			) {
