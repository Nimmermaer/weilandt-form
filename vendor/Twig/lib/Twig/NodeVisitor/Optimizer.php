...
class Twig_NodeVisitor_Optimizer extends Twig_BaseNodeVisitor {
	...
	protected function doEnterNode( Twig_Node $node, Twig_Environment $env ) {
		...
		if ( PHP_VERSION_ID < 50400 &&
		     self::OPTIMIZE_VAR_ACCESS === ( self::OPTIMIZE_VAR_ACCESS & $this->optimizers ) &&
	...
	protected function doLeaveNode( Twig_Node $node, Twig_Environment $env ) {
		...
		if ( self::OPTIMIZE_VAR_ACCESS === ( self::OPTIMIZE_VAR_ACCESS & $this->optimizers ) &&
		     ! $env->isStrictVariables() && ! $env->hasExtension( 'sandbox' )
		) {
			if ( $node instanceof Twig_Node_Body ) {
				$this->inABody = FALSE;
			} elseif ( $this->inABody ) {
				if ( ! $expression && get_class( $node ) !== 'Twig_Node' &&
	...
	protected function enterOptimizeFor( Twig_NodeInterface $node, Twig_Environment $env ) {
		...
		elseif ( $node instanceof Twig_Node_Expression_Function && 'include' === $node->getAttribute( 'name' ) &&
