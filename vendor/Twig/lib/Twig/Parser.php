...
class Twig_Parser implements Twig_ParserInterface {
	...
	public function parse( Twig_TokenStream $stream, $test = NULL, $dropNeedle = FALSE ) {
		...
		$node = new Twig_Node_Module( new Twig_Node_Body( array( $body ) ),
		                              $this->parent,
			...
			switch ( $this->getCurrentToken()->getType() ) {
				...
				case Twig_Token::BLOCK_START_TYPE:
					...
					if ( $token->getType() !== Twig_Token::NAME_TYPE ) {
						throw new Twig_Error_Syntax( 'A block must start with a tag name.',
						                             $token->getLine(),
						                             $this->getFilename() );
					}
					...
					if ( NULL === $subparser ) {
						if ( NULL !== $test ) {
							...
							if ( is_array( $test ) && isset( $test[0] ) &&
							     $test[0] instanceof Twig_TokenParserInterface
							) {
	...
	protected function filterBodyNodes( Twig_NodeInterface $node ) {
		...
		if ( ( $node instanceof Twig_Node_Text && ! ctype_space( $node->getAttribute( 'data' ) ) ) ||
			...
			throw new Twig_Error_Syntax( 'A template that extends another one cannot have a body.',
			                             $node->getLine(),
			                             $this->getFilename() );
		}
