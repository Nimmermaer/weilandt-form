...
class Twig_Tests_Node_Expression_FilterTest extends Twig_Test_NodeTestCase {
	...
	public function getTests() {
		...
		$environment->addFilter( new Twig_SimpleFilter( 'barbar',
		                                                'twig_tests_filter_barbar',
		                                                array( 'needs_context' => TRUE, 'is_variadic' => TRUE ) ) );

		$tests = array();

		$expr = new Twig_Node_Expression_Constant( 'foo', 1 );
		$node = $this->createFilter( $expr, 'upper' );
		$node = $this->createFilter( $node,
		                             'number_format',
		                             array(
		...
		$node    = $this->createFilter( $string,
		                                'barbar',
		                                array( 'foo' => new Twig_Node_Expression_Constant( 'bar', 1 ) ) );
		$tests[] = array(
			$node,
		...
		$node    = $this->createFilter( $string,
		                                'barbar',
		                                array( 'arg2' => new Twig_Node_Expression_Constant( 'bar', 1 ) ) );
