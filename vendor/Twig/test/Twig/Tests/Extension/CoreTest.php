...
class Twig_Tests_Extension_CoreTest extends PHPUnit_Framework_TestCase {
	...
	public function testRandomFunction( $value, $expectedInArray ) {
		...
		for ( $i = 0; $i < 100; ++ $i ) {
			$this->assertTrue( in_array( twig_random( $env, $value ),
			                             $expectedInArray,
	...
	public function testRandomFunctionReturnsAsIs() {
		...
		$this->assertSame( '',
		                   twig_random( new Twig_Environment( $this->getMock( 'Twig_LoaderInterface' ),
		                                                      array( 'charset' => NULL ) ),
		                                '' ) );

		$instance = new stdClass();
		$this->assertSame( $instance,
		                   twig_random( new Twig_Environment( $this->getMock( 'Twig_LoaderInterface' ) ), $instance ) );
	}
	...
	public function testRandomFunctionOnNonUTF8String() {
		...
		for ( $i = 0; $i < 30; ++ $i ) {
			...
			$this->assertTrue( in_array( twig_convert_encoding( $rand, 'UTF-8', 'ISO-8859-1' ),
			                             array( 'Ä', 'é' ),
			                             TRUE ) );
		}
	}
