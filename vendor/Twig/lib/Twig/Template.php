...
abstract class Twig_Template implements Twig_TemplateInterface {
	...
	public function getEnvironment() {
		@trigger_error( 'The ' . __METHOD__ . ' method is deprecated since version 1.20 and will be removed in 2.0.',
		                E_USER_DEPRECATED );
	...
	protected function getAttribute( $object, $item, array $arguments = array(), $type = self::ANY_CALL, $isDefinedTest = FALSE, $ignoreStrictCheck = FALSE ) {
		...
		if ( self::METHOD_CALL !== $type ) {
			...
			if ( self::ARRAY_CALL === $type || ! is_object( $object ) ) {
				...
				if ( $object instanceof ArrayAccess ) {
					$message = sprintf( 'Key "%s" in object with ArrayAccess of class "%s" does not exist',
					                    $arrayItem,
					                    get_class( $object ) );
					...
					if ( empty( $object ) ) {
						...
						$message = sprintf( 'Key "%s" for array with keys "%s" does not exist',
						                    $arrayItem,
					...
					if ( NULL === $object ) {
						...
						$message = sprintf( 'Impossible to access a key ("%s") on a %s variable ("%s")',
						                    $item,
						                    gettype( $object ),
						                    $object );
					}
					...
					$message = sprintf( 'Impossible to access an attribute ("%s") on a %s variable ("%s")',
					                    $item,
					                    gettype( $object ),
					                    $object );
				}
		...
		if ( ! is_object( $object ) ) {
			...
			if ( NULL === $object ) {
				...
				$message = sprintf( 'Impossible to invoke a method ("%s") on a %s variable ("%s")',
				                    $item,
				                    gettype( $object ),
				                    $object );
			}
		...
		if ( isset( self::$cache[ $class ]['methods'][ $lcItem ] ) ) {
			...
			throw new Twig_Error_Runtime( sprintf( 'Method "%s" for object "%s" does not exist',
			                                       $item,
