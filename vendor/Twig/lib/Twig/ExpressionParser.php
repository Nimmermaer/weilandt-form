...
class Twig_ExpressionParser {
	...
	public function parseExpression( $precedence = 0 ) {
		...
		while ( $this->isBinary( $token ) &&
		        $this->binaryOperators[ $token->getValue() ]['precedence'] >= $precedence ) {
			...
			if ( isset( $op['callable'] ) ) {
				...
				$expr1 = $this->parseExpression( self::OPERATOR_LEFT === $op['associativity'] ? $op['precedence'] +
				                                                                                1 : $op['precedence'] );
				$class = $op['class'];
	...
	protected function parseConditionalExpression( $expr ) {
		while ( $this->parser->getStream()->nextIf( Twig_Token::PUNCTUATION_TYPE, '?' ) ) {
			...
			$expr = new Twig_Node_Expression_Conditional( $expr,
			                                              $expr2,
			                                              $expr3,
	...
	public function parsePrimaryExpression() {
		...
		switch ( $token->getType() ) {
			...
			case Twig_Token::OPERATOR_TYPE:
				if ( preg_match( Twig_Lexer::REGEX_NAME, $token->getValue(), $matches ) &&
				     $matches[0] == $token->getValue()
				) {
					...
					if ( ! ( in_array( $ref->getName(), array( $negClass, $posClass ) ) ||
		...
		switch ( $name ) {
			case 'parent':
				...
				if ( ! count( $this->parser->getBlockStack() ) ) {
					throw new Twig_Error_Syntax( 'Calling "parent" outside a block is forbidden.',
					                             $line,
					                             $this->parser->getFilename() );
				}
		...
		if ( $token->getValue() == '.' ) {
			...
			if ( $token->getType() == Twig_Token::NAME_TYPE || $token->getType() == Twig_Token::NUMBER_TYPE ||
			     ( $token->getType() == Twig_Token::OPERATOR_TYPE &&
			       preg_match( Twig_Lexer::REGEX_NAME, $token->getValue() ) )
			) {
			...
			if ( $node instanceof Twig_Node_Expression_Name &&
				...
				if ( $this->parser->isReservedMacroName( $name ) ) {
					throw new Twig_Error_Syntax( sprintf( '"%s" cannot be called as macro as it is a reserved keyword.',
	...
	protected function getFunctionNodeClass( $name, $line ) {
		...
		if ( FALSE === $function = $env->getFunction( $name ) ) {
			$e = new Twig_Error_Syntax( sprintf( 'Unknown "%s" function.', $name ),
			                            $line,
			                            $this->parser->getFilename() );
